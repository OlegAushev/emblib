...
public:
  ...
  constexpr inplace_stack(inplace_stack&& other) noexcept(
  ...
  constexpr inplace_stack& operator=(inplace_stack&& other) noexcept(
      std::is_nothrow_move_constructible_v<T>)
    requires(std::is_move_constructible_v<T>
             && !std::is_trivially_copyable_v<T>)
  {
...
private:
  ...
  constexpr void move_construct_from(inplace_stack& other) noexcept(
      std::is_nothrow_move_constructible_v<T>)
  {
    for (size_type i = 0; i < other.size_; ++i) {
