...
template<typename F>
concept some_hash_fn =
    requires { typename F::type; } && requires(F f, char const* s) {
...
concept some_parameter = requires {
  ...
  requires std::same_as<std::remove_cv_t<decltype(P::default_value)>,
                        typename P::value_type>;
  P::name.size();
};
...
struct basic_layout {
  ...
  static consteval std::size_t checked_index_of()
  {
    ...
    if constexpr (i != SIZE_MAX) {
      ...
      static_assert(parameter_at<i>::name == Name,
...
public:
  ...
  static_assert(Layout.base + Layout.size() <= Storage::capacity,
  ...
  class parameter_ref {
    ...
    constexpr parameter_ref(addr_type hash_loc,
                            addr_type val_loc,
                            addr_type crc_loc,
                            hash_type hash)
        : hash_loc_(hash_loc), val_loc_(val_loc), crc_loc_(crc_loc), hash_(hash)
    {
    }
  ...
  auto erase() -> std::expected<void, error>
  {
    for (std::size_t off = Layout.base; off < Layout.base + size; ++off) {
      auto r =
          storage_.template write<std::byte>(typename Storage::addr_type(off),
                                             std::byte{0});
      if (!r) return r;
    }
    return {};
