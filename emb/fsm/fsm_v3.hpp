...
template<typename... States>
concept states_with_unique_ids = are_ids_unique<States...>();
...
concept state_event_handler =
    ...
    && requires(typename Policy::template event_context_type<Context> ctx,
                Event&& event) {
         {
...
struct diagnose_fsm_details<Context, Policy, true, States...> {
  static_assert(states_with_same_id_type<States...>,
                "finite_state_machine: all State::id must share the same type");
  static_assert(states_with_unique_ids<States...>,
...
struct diagnose_fsm {
  static_assert(always_false<StateList>,
...
struct diagnose_fsm<Derived, Policy, typelist<States...>> {
  ...
  static constexpr bool ok = diagnose_fsm_details < Derived, Policy,
                        fsm_policy<Policy, Derived>
                            && (state_with_id<States> && ...),
                        States... > ::ok;
};
...
public:
  ...
  template<typename Event>
    requires(detail::event_handler<context_type,
                                   Policy,
                                   Event,
                                   next_state_type,
                                   States...>)
  constexpr void dispatch(Event&& event)
  {
    auto const visitor = [&](auto const& s) -> next_state_type {
      ...
      if constexpr (detail::state_event_handler<S,
                                                context_type,
                                                Policy,
                                                Event,
                                                next_state_type>) {
      ...
      else if constexpr (detail::common_event_handler<context_type,
                                                      Policy,
                                                      Event,
                                                      next_state_type>) {
