...
void emb::tests::chrono_test() {
  ...
  emb::chrono::milliseconds msec =
      emb::chrono::duration_cast<emb::chrono::milliseconds>(sec);
  EMB_ASSERT_EQUAL(msec.count(), 10000);
  EMB_ASSERT_EQUAL(msec, emb::chrono::milliseconds(10000));

  emb::chrono::microseconds usec =
      emb::chrono::duration_cast<emb::chrono::microseconds>(sec);
  EMB_ASSERT_EQUAL(usec.count(), 10000000);

  EMB_MAYBE_UNUSED emb::chrono::nanoseconds nsec =
      emb::chrono::duration_cast<emb::chrono::nanoseconds>(usec);
  EMB_ASSERT_EQUAL(nsec.count(), 10000000000);

  EMB_ASSERT_EQUAL(msec - emb::chrono::milliseconds(1),
                   emb::chrono::milliseconds(9999));

  EMB_MAYBE_UNUSED emb::chrono::milliseconds msec2 =
      msec + emb::chrono::milliseconds(1);
  ...
  EMB_MAYBE_UNUSED emb::chrono::seconds sec2 = ++sec;
  EMB_ASSERT_EQUAL(sec.count(), 11);
  EMB_ASSERT_EQUAL(sec2.count(), 11);

  EMB_MAYBE_UNUSED emb::chrono::seconds sec3 = sec++;
  EMB_ASSERT_EQUAL(sec.count(), 12);
  EMB_ASSERT_EQUAL(sec3.count(), 11);

  EMB_MAYBE_UNUSED emb::chrono::seconds sec4 = --sec;
  EMB_ASSERT_EQUAL(sec.count(), 11);
  EMB_ASSERT_EQUAL(sec4.count(), 11);

  EMB_MAYBE_UNUSED emb::chrono::seconds sec5 = sec--;
  EMB_ASSERT_EQUAL(sec.count(), 10);
